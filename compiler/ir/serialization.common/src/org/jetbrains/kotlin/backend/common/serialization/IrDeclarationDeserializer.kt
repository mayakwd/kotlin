...
class IrDeclarationDeserializer(
        ...
        withDeserializedIrDeclarationBase(proto.base) { symbol, signature, startOffset, endOffset, origin, fcode ->
            ...
            symbolTable.declareClass(signature, { symbol as IrClassSymbol }) {
                irFactory.createClass(
                    ...
                    flags.visibility,
                    flags.modality,
                    deserializeInlineClassRepresentation(proto),
                    flags.isCompanion,
                    flags.isInner,
                    flags.isData,
    ...
    private fun deserializeInlineClassRepresentation(proto: ProtoClass): InlineClassRepresentation<IrSimpleType>? {
        if (!proto.hasInlineClassUnderlyingPropertyName()) return null
        check(proto.hasInlineClassUnderlyingPropertyType()) {
            "Inline class representation should have both name and type: ${deserializeName(proto.name)}"
        }
        val name = deserializeName(proto.inlineClassUnderlyingPropertyName)
        val type = deserializeIrType(proto.inlineClassUnderlyingPropertyType) as IrSimpleType
