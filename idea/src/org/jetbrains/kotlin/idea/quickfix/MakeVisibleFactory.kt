...
object MakeVisibleFactory : KotlinIntentionActionsFactory() {
    ...
    override fun doCreateActions(diagnostic: Diagnostic): List<IntentionAction> {
        ...
        val isSealedClassConstructor = descriptor is ClassConstructorDescriptor && descriptor.constructedClass.isSealed()
        if (isSealedClassConstructor && !suggestProtected(declaration)) return emptyList()

        val module = DescriptorUtils.getContainingModule(descriptor)
        val targetVisibilities = when (descriptor.visibility) {
            PRIVATE, INVISIBLE_FAKE -> {
                if (isSealedClassConstructor) {
                    listOf(PROTECTED)
                } else {
                    mutableListOf(PUBLIC).apply {
    ...
    private fun suggestProtected(declaration: PsiElement): Boolean = declaration.languageVersionSettings
